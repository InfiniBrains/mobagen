...
void CoolGame::init(GLManager *glManager)
{
  ...
  md5mesh->getEntity()->getTransform().setPosition(glm::vec3(3, 1, -2));
  md5mesh->getEntity()->getTransform().setScale(glm::vec3(.05, .05, .05));
  md5mesh->getEntity()->getTransform().rotate(glm::vec3(1, 0, 0), glm::pi<float>() / 2.f);
...
{
  CoolGame game;
  Engine gameEngine(&game, "Animation Demo", glm::vec2(800,600));
