#include "Knight.h"
unordered_set<Point2D> Knight::AttackMoves(WorldState& world, const Point2D& origin) {
...
unordered_set<Point2D> Knight::CoverMoves(WorldState& world, const Point2D& origin) {
  auto piece = world.PieceAtPosition(origin);
  if (piece.Piece() != PieceType::Knight) return {};
  ...
  for (auto delta : deltas) {
    ...
    if (other.Piece() == PieceType::WRONG ||  // outside board
        ((uint8_t)other.Piece() < (uint8_t)PieceType::WRONG && other.Piece() != PieceType::NONE
         && other.Color() != piece.Color()))  // other side piece
      continue;
    moves.insert(sum);
  }
  return moves;
