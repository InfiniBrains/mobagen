...
float Vector2::getAngleDegree(Vector2 v) { return v.getAngleDegree(); }
...
bool Vector2::operator!=(const Vector2& rhs) const { return (*this - rhs).sqrMagnitude() >= 1.0e-6; }
...
Vector2 Vector2::Rotate(Vector2 up) const { return this->Rotate(up.getAngleDegree()); }

Vector2 Vector2::Random(float start, float end) { return {Random::Range(start, end), Random::Range(start, end)}; }

float Vector2::getAngleRadian() const { return atan2(x, -y); }

float Vector2::getAngleRadian(Vector2 v) { return v.getAngleRadian(); }

Vector2 Vector2::getVector2FromRadian(const float radian) { return {cos(radian), sin(radian)}; }
...
float Vector2::getMagnitude() const { return sqrt(sqrMagnitude()); }

float Vector2::getMagnitude(const Vector2& vector) { return vector.getMagnitude(); }
...
float Vector2::getSquaredDistance(const Vector2& a, const Vector2& b) { return (a.x - b.x) * (a.x - b.x) + (a.y - b.y) * (a.y - b.y); }

Vector2 Vector2::normalized(const Vector2& vector) { return vector.normalized(); }
