...
namespace mobagen {
  ...
  private:
    ...
    aiQuaternion
    interpolateFunction(const aiQuaternion &beforeMatrix, const aiQuaternion &afterMatrix, double factor) const {
  ...
  public:
    ...
    auto interpolate(const std::vector<KeyType> &keys, double time,
                     bool loop = true) const -> decltype(keys.begin()->mValue) {
      time *= ticksPerSecond;
  ...
  public:
    ...
    public:
      ...
      MeshFrame(const MeshExtended &mesh) : vertices(mesh.vertices.size()), normals(mesh.normals.size()),
                                            tangents(mesh.tangents.size()), mesh(mesh) {}
    };

    ///////////////////////
    aiMatrix4x4
    aiMatrix4x4Compose(const aiVector3D &scaling, const aiQuaternion &rotation, const aiVector3D &position) {
      aiMatrix4x4 r;
