...
class PieceBase {
  ...
  virtual vector<Point2D> PossibleMoves(const WorldState& world, const Point2D& origin) = 0;
  // GenerateStates to be used when we explcout << a << " ";ore
  virtual vector<WorldState> GenerateStates(const WorldState& world, const Point2D& origin) = 0;
};
