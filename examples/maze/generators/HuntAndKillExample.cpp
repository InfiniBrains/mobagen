...
bool HuntAndKillExample::Step(World* w) {
  ...
  if (stack.empty()) {
    ...
    if (point.x == INT_MAX && point.y == INT_MAX) return false;  // no empty space no fill
    ...
    if (point.x != -w->GetSize() / 2 || point.y != -w->GetSize() / 2) {
      ...
      if (visitedNeighbors.empty()) return false;  // this should never happen. if we are in this state, the code is wrong
      auto next = visitedNeighbors[Random::Range(0, visitedNeighbors.size() - 1)];
  ...
  if (visitables.empty()) {
    for (auto v : stack) w->SetNodeColor(v, Color::Black);
    stack.clear();
    return true;
  } else {  // go deeper
